...
const victimData = z.object({
    ...
    data_nascimento:            z.nullable(z.string()).describe(`Data de nascimento da vítima.`),
    email_contato:              z.nullable(z.string()).describe(`Email de contato da vítima.`),
    tel_contato:                z.nullable(z.string()).describe(`Telefone de contato da vítima.`),
});
...
const offenderData = z.object({
    ...
    data_nascimento:            z.nullable(z.string()).describe(`Data de nascimento de que cometeu o delito.`),
    ...
    envolvimento:               z.nullable(z.string()).describe(`Envolvimento da pessoa na occorrência.`),
    condicao_envolvido:         z.nullable(z.string()).describe(`Condição física do envolvido.`),
});
...
const crimesCommited = z.nullable(z.string()).describe(`Tipo de crime efetuado na ocorrência seguindo o padrão: Roubo, Furto, Vias de fato, Violência contra a mulher`);
...
const locationData = z.object({
    ...
    nome_municipio:             z.nullable(z.string()).describe(`Nome do munícipio em que aconteceu a ocorrência.`),
    sigla_uf:                   z.nullable(z.string()).describe(`Sigla da unidade federal em que aconteceu a ocorrência.`),
    endereco_logradouro:        z.nullable(z.string()).describe(`Nome logradouro em que aconteceu a ocorrência.`),
    endereco_numero:            z.nullable(z.number()).describe(`Número do endereço em que aconteceu a ocorrência.`),
    endereco_bairro:            z.nullable(z.string()).describe(`Nome do bairro em que aconteceu a ocorrência.`),
    endereco_complemento:       z.nullable(z.string()).describe(`Complemento do endereço em que aconteceu a ocorrência.`),
    endereco_ponto_referencia:  z.nullable(z.string()).describe(`Ponto de referência para o local em que aconteceu a ocorrência.`),
    nome_rodovia:               z.nullable(z.string()).describe(`Nome da rodovia em que aconteceu a ocorrência.`),
    numero_km:                  z.nullable(z.number()).describe(`Número do quilometro.`),
    texto_trecho:               z.nullable(z.string()).describe(`Trecho em que aconteceu a ocorrência.`), // ??? Mudar isso o quanto antes.
...
const dateTimeData = z.object({
    ...
    data_acontecimento_ocorrencia:      z.nullable(z.string()).describe(`Data em que aconteceu a ocorrência no formato dd/mm/aaaa.`),
    horario_acontecimento_ocorrencia:   z.nullable(z.string()).describe(`Horário em que aconteceu a ocorrência no formato hh:mm`)
});
